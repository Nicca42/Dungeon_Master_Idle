...
import { encounterX, slotX, EXIT_CENTER, REST_CENTER, ENTRY_CENTER } from '../game/layout';
const WIDTH = 640;
...
function ActionBubble({ actions, reduced }: { actions: ActionCue[]; reduced: boolean }) {
  ...
  return (
    ...
    >
      <Row style={{ gap: 7 }}>
        <Text style={{ fontFamily: fonts.medium, color: c.red, fontSize: 16 }}>{a.difficulty}</Text>
        <Icon size={16} color={a.success ? c.gold : c.red} />
        <Text style={{ fontFamily: fonts.medium, color: c.green, fontSize: 16 }}>{a.power}</Text>
      </Row>
      <Text style={{ fontFamily: fonts.body, color: c.text, fontSize: 8 }}>
        {a.actor} · {a.kind}
      </Text>
    </Animated.View>
  );
...
function SurfaceParty({ g, p, reduced }: { g: GameState; p: Party; reduced: boolean }) {
  const progress = Math.min(1, Math.max(0, 1 - (p.surfaceUntil - g.now) / (3 * TICK)));
...
function SurfaceLife({
  ...
  const waiting = g.opened ? town.filter((a) => a.status === 'town') : [];
        ...
        >
          ...
          <Spinner
            progress={
              g.tutorial < 10
                ? 0
                : Math.max(0, 1 - (arrivalAt(g, point) - g.now) / (rule(g, 'arrivalHours') * HOUR))
            }
      ...
      >
        ...
        >
          ...
          >
            ...
            <Body>
              {waiting.length} waiting for a party. Nobody leaves because they have waited too long.
            </Body>
            <Body>
              Only six sprites are shown outside. Training and recovering adventurers return to the
              queue when ready.
            </Body>
            <ScrollView>
              {town.map((a) => (
...
function PartySprites({
  ...
  return (
    ...
    >
      {p.actions.length > 0 && (
        <View style={{ position: 'absolute', bottom: 36, left: -34 }}>
...
function MobileMob({
  ...
  return (
    <Animated.View
      style={{ position: 'absolute', bottom: 17, left: 0, transform: [{ translateX: x }] }}
    >
...
function FloorCanvas({
  ...
  const crowded = mobCount > 0 && mobLimitReached(g, f);
  const warningColor = mobCount > (g.policy.mobLimit ?? g.policy.mobSlots) ? c.red : c.gold;
  return (
    <View style={{ height: 148, backgroundColor: '#302934', position: 'relative' }}>
      <Modal
        visible={!!trap}
        transparent
        animationType="fade"
      ...
      {crowded && (
        <View
          ...
          accessibilityLabel={`${mobCount > (g.policy.mobLimit ?? g.policy.mobSlots) ? 'Red' : 'Yellow'} ! Mob limit reached ${mobCount}/${g.policy.mobLimit ?? g.policy.mobSlots}`}
          style={{
            position: 'absolute',
            top: 24,
            left: 140,
        ...
        >
          <Text style={{ color: warningColor, fontSize: 18, fontFamily: fonts.bold }}>!</Text>
          <Text style={{ color: warningColor, fontSize: 10, fontFamily: fonts.medium }}>
            Mob limit reached {mobCount}/{g.policy.mobLimit ?? g.policy.mobSlots}
          </Text>
        </View>
      )}
      <View style={{ position: 'absolute', bottom: 0, left: 0, right: 0 }}>
        {built ? (
          <FloorArt built digging={false} index={f.id} level={f.level} encounters={NO_ENCOUNTERS} />
        ) : (
      ...
      {built ? (
        <>
          <Cave x={EXIT_CENTER} y={66} entry={false} locked={f.stage !== 'open'} />
          {built && <Cave x={ENTRY_CENTER} y={66} entry locked={f.stage !== 'open'} />}
          <Svg width={640} height={148} style={{ position: 'absolute' }}>
            {[640 / 3, 1280 / 3].map((x) => (
              <Path
                key={x}
                d={`M${x} 38v96`}
          ...
          >
            {waitingParties.slice(0, 3).map((p) => (
              <Text key={p.id} style={{ fontFamily: fonts.bold, fontSize: 9, color: c.green }}>
                {
                  p.members.filter((id) => {
            ...
            {waitingParties.length > 3 && (
              <Text style={{ fontSize: 8, color: c.gold }}>4+ parties waiting</Text>
            )}
          </View>
          {f.encounters
            .filter(
            ...
            .map((e) => (
              <View
                ...
                style={{
                  position: 'absolute',
                  top: 8,
                  left: routeX(f, f.encounters.indexOf(e)) + 15 - 22,
                  width: 44,
                  alignItems: 'center',
                }}
              >
                {['wood', 'silver', 'gold'].includes(e.kind) ? (
                  ...
                  <>
                    <Text style={{ fontFamily: fonts.medium, color: c.text, fontSize: 8 }}>
                      {e.kind === 'trapdoor'
                        ? 'Trapdoor'
                        : e.kind === 'arrows'
                          ? 'Arrows'
                    ...
                    <Text style={{ fontFamily: fonts.medium, color: c.muted, fontSize: 8 }}>
                      DMG {e.damage}
                    </Text>
                    {e.kind === 'mimic' && (
                      <Text style={{ fontFamily: fonts.medium, color: c.purple, fontSize: 8 }}>
                        XP {e.xp ?? 0}
                      </Text>
                    )}
                  </>
            ...
            <>
              ...
              >
                <Text
                  ...
                  style={{ fontFamily: fonts.bold, color: c.red, fontSize: 9 }}
                >
                  {restWaiters(g, f.id).length}
                </Text>
                <Text
                  testID={`rest-sign-${f.id}`}
                  style={{ fontFamily: fonts.bold, color: '#fff4de', fontSize: 8 }}
                >
                  REST
                </Text>
                <Text style={{ fontFamily: fonts.medium, color: c.green, fontSize: 8 }}>
            ...
            .map((e) => (
              <React.Fragment key={e.id}>
                ...
                {!e.roaming && (
                  ...
                  >
                    <View accessibilityLabel={`${e.kind} spawner`} style={{ opacity: 0.4 }}>
                      <Sprite
                        kind={e.kind as SpriteKind}
                        variant={e.variant}
                        saturation={e.saturation}
                    ...
                    {(e.active || mobLimitReached(g, f)) && (
                      <Text style={{ fontSize: 8, color: c.gold }}>
                        {mobLimitReached(g, f) ? 'At limit' : 'Mob active'}
                      </Text>
                    )}
                  </View>
                )}
              ...
              >
                ...
                >
                  <HealthBars
                    ...
                    maxDefense={a.maxDefense}
                  />
                  <View style={{ width: 28, marginBottom: 3 }}>
                    <Progress value={a.stamina / a.maxStamina} color={c.green} height={3} />
                  </View>
                  {a.task && (a.task.arriveAt ?? 0) <= g.now && <Hammer size={13} color={c.gold} />}
                  <Sprite
                    kind="maintenance"
                    size={30}
...
function FloorHeaderStatus({ g, f }: { g: GameState; f: Floor }) {
  ...
  if (f.upgradeWork !== undefined)
    return (
      <View style={{ flex: 1, marginLeft: 10 }} testID={`upgrade-progress-${f.id}`}>
        <Progress value={f.upgradeWork / (f.upgradeRequired ?? 6)} color={c.purple} height={5} />
        <Body style={{ fontSize: 10 }}>
          {upgradingFloor(g)?.id === f.id ? 'Building level 2' : 'Upgrade queued · deepest first'}
        </Body>
      </View>
    );
  if (!['queued', 'excavating', 'foundation', 'furnishing'].includes(f.stage))
    return <Label color={f.stage === 'open' ? c.green : c.dim}>{f.stage}</Label>;
  return (
    <View
    ...
    >
      ...
      <Text style={{ color: c.gold, fontFamily: fonts.medium, fontSize: 10 }}>
        {eta === null ? 'Awaiting crew' : `${countdown(eta - g.now)} remaining`} · {f.stage}
      </Text>
    </View>
  );
}
                  ...
                  <View key={f.id} style={{ zIndex: 5 - i }}>
                    ...
                    >
                      <Row style={{ justifyContent: 'space-between' }}>
                        <Text style={{ fontFamily: fonts.medium, color: c.text, fontSize: 14 }}>
