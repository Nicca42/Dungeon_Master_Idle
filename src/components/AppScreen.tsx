...
import { restRoster } from '../game/rest';
...
const tabs = [
  ...
  { id: 'art' as Tab, title: 'Art', icon: Sparkles, sub: 'Choose your pixel art' },
];
...
export default function AppScreen() {
  ...
  return (
    <View style={s.root}>
        ...
        <View style={[s.topbar, { paddingHorizontal: desktop ? 32 : 18 }]}>
          ...
          <Row style={{ gap: desktop ? 20 : 10 }}>
            ...
            <Pressable
              ...
              onPress={() => setModal('demo')}
              style={s.iconButton}
            >
              <Settings2 size={18} color={c.muted} />
            </Pressable>
          </Row>
        </View>
        <ScrollView
          ...
          <View style={{ maxWidth: 1400, alignSelf: 'center', width: '100%' }}>
            {(actualTab !== 'dungeon' || welcome) && (
              <Row style={{ justifyContent: 'space-between', alignItems: 'flex-start', gap: 12 }}>
                ...
                {desktop && (
                  <Button secondary icon={FastForward} onPress={() => setModal('demo')}>
                    Demo controls
                  </Button>
                )}
              </Row>
            ...
            {g.gameOver && (
              <Panel>
                ...
                <Button onPress={() => setModal('reset')}>Restart game</Button>
              </Panel>
            )}
            {actualTab !== 'art' && (
            ...
            >
              ...
              >
                {g.tutorial < 10 ? (
                  ...
                  <Pulse g={g} onDemo={() => setModal('demo')} />
                )}
              </View>
              <View
                style={{
      ...
      >
        <Body>Time until full stamina (dungeon HH:MM). Healing can extend a stay.</Body>
        {restFloor !== null &&
          restRoster(g, restFloor).map(({ actor, end }) => (
            <Panel key={actor.id}>
              <Heading size={18}>
                {actor.name} · {actor.role}
              </Heading>
              <Body>
                Stamina {actor.stamina}/{actor.maxStamina} · {countdown(end - liveNow)} remaining
              </Body>
            </Panel>
          ))}
        {restFloor !== null && restRoster(g, restFloor).length === 0 && (
          <Body>No occupants right now.</Body>
        )}
      </Dialog>
      <Dialog
      ...
      >
              ...
              ((modal === 'admin' && ![3, 4, 5, 6].includes(g.tutorial)) ||
                ...
                <Panel>
                  ...
                  {[1, 2, 7].includes(g.tutorial) ? (
                    ...
                    <Button disabled={busy} onPress={() => void finishTutorial()}>
                      ...
                      <>
                        ...
                        {staffPage === 'diggers' && (
                          <>
                            ...
                            <Body>
                              {g.excavationSpells} free excavation spells left. Each finishes
                              digging only; foundations and furnishings still need builders. Floors
                              are worked in order.
                            </Body>
                            {g.floors
                              ...
                              .map((f) => (
                                <Button
                                  ...
                                  onPress={() =>
                                    void dispatch({ type: 'excavationSpell', floor: f.id })
                                  }
                                >
        ...
        {modal === 'demo' && (
          <>
            <Body>
              These controls are just for the demo. Time skips use the same simulation as being
              away.
            </Body>
            <Divider />
            ...
            <Body style={{ marginTop: 16, fontSize: 10 }}>
              Build v0.1 · Seed {g.seed} · {g.actors.length} NPCs · {g.parties.length} active
              parties
            </Body>
          </>
        )}
        {(modal === 'reset' || modal === 'fixture') && (
          <>
            ...
            <Row style={{ marginTop: 24 }}>
              <Button secondary onPress={() => setModal('demo')}>
                Keep my dungeon
              </Button>
              <Button
                icon={RotateCcw}
      ...
      >
        {report && (
          <>
            ...
            <Button onPress={dismissReport} style={{ marginTop: 20 }} icon={ArrowRight}>
              Back to the dungeon
            </Button>
          </>
        )}
...
function Tutorial({
  ...
  return (
    <Panel style={{ borderColor: '#65503b', backgroundColor: '#28232a', padding: 22 }}>
      ...
      <Button disabled={busy} testID="tutorial-action" onPress={onOpen} icon={ArrowRight}>
        {t.action}
      </Button>
...
function Pulse({ g, onDemo }: { g: GameState; onDemo: () => void }) {
  return (
    <Panel style={{ borderColor: '#4e5c4d', backgroundColor: '#232a27' }}>
      ...
      <Button onPress={onDemo} secondary icon={FastForward} style={{ marginTop: 20 }}>
        ...
        <Panel>
          ...
          <Row>
            ...
            {tip < 3 ? (
              ...
              <Button disabled={busy || g.gold < 2000} onPress={() => onComplete?.()}>
