...
export function applyRules(state: GameState, input: Rules): GameState {
  ...
  if (changed('arrivalHours'))
    next.nextArrivalAt =
      next.now +
      (Math.max(0, next.nextArrivalAt - next.now) * rule(next, 'arrivalHours')) /
        rule(state, 'arrivalHours');
  for (const a of next.actors) {
    const old = state.actors.find((x) => x.id === a.id)!;
    const speedRatio = Math.max(1, old.speed) / Math.max(1, a.speed);
    if (a.task && (changed('maintenanceHours') || speedRatio !== 1)) {
      ...
      a.task.until =
        next.now +
        Math.ceil(
          (travel + (work * rule(next, 'maintenanceHours')) / rule(state, 'maintenanceHours')) *
            speedRatio,
        );
    }
