import { maintenanceTiming, staffTravelTime } from './staffSpeed';
...
import { nextLook } from './appearance';
...
import { rule, DEFAULT_RULES, Rules, validateRules } from './config';
...
export function initialState(seed = 42691, config: Rules = DEFAULT_RULES): GameState {
  ...
  s.nextArrivalAt = rule(s, 'arrivalHours') * HOUR;
  for (const role of [
    'fighter',
    'fighter',
    'fighter',
...
function admit(s: GameState) {
  if (!s.opened || s.gameOver || s.parties.length >= 12) return;
  const available = s.actors.filter(
    (a) =>
      a.status === 'town' && level(a) >= 1 && level(a) <= s.floors[0]!.level && a.wealth >= s.fee,
  );
  ...
  if (fighters.length < 2 || !wizard || !healer) return;
...
function assignMaintenance(s: GameState, resetsOnly = true) {
  for (const a of s.actors.filter((a) => a.role === 'maintenance' && a.health > 0)) {
    ...
    if (work) {
      ...
      const timing = maintenanceTiming(s, a, f.id, kind === 'reset' ? (e.tier ?? 1) : 1);
      a.workFloor = f.id;
...
export const adventurerCap = (s: GameState) => rule(s, 'populationCap') + (s.spawnPoints - 1) * 10;
const adventurerRoles = ['fighter', 'wizard', 'healer'] as const;
const partyRequirement = { fighter: 2, wizard: 1, healer: 1 };
function waitingPartyCounts(s: GameState) {
...
export function guildPartyForecast(
  ...
  if (s.parties.length >= 12) return { remaining: null, reason: 'Party limit reached' };
  const counts = waitingPartyCounts(s);
  const originalCounts = { ...counts };
  const missing = adventurerRoles.reduce(
    (n, r) => n + Math.max(0, partyRequirement[r] - counts[r]),
    0,
  );
  const rate = active ? 2 : 1;
  const firstDeadline = s.now + Math.max(0, s.nextArrivalAt - s.now) / rate;
  const firstTick = s.nextTick + Math.max(0, Math.ceil((firstDeadline - s.nextTick) / TICK)) * TICK;
  if (!missing) return { remaining: Math.max(0, firstTick - s.now) };
  const population = s.actors.filter(
    (a) =>
      adventurerRoles.includes(a.role as (typeof adventurerRoles)[number]) && a.status !== 'dead',
  );
  if (population.length + missing > adventurerCap(s))
    return { remaining: null, reason: 'Population limit reached' };
  for (let i = 0; i < missing; i++) {
    const point = (arrivalSequence(s) + i) % s.spawnPoints;
    const role = guildChoice(counts);
    const tier = s.spawnTiers?.[point] ?? 1;
    const prefix = tier === 2 ? `level2.${role}` : role;
    const spawnLevel = Math.floor(
      Math.min(
        ...[
          'maxHealth',
          'maxDefense',
          ...
          'speed',
          'primary',
        ].map((stat) => rule(s, `${prefix}.${stat}`)),
      ) / 10,
    );
    if (
      spawnLevel < 1 ||
      spawnLevel > s.floors[0]!.level ||
      rule(s, `${prefix}.wealth`) * 100 < s.fee
    )
      return { remaining: null, reason: 'Spawn settings fail door policy' };
    const cap = Math.ceil((adventurerCap(s) * partyRequirement[role]) / 4);
    if (
      population.filter((a) => a.role === role).length +
        Math.max(0, counts[role] - originalCounts[role]) >=
      cap
    )
      return { remaining: null, reason: `${role} population limit` };
    counts[role]++;
  }
  const interval = Math.ceil((rule(s, 'arrivalHours') * HOUR) / s.spawnPoints / rate / TICK) * TICK;
  return { remaining: Math.max(0, firstTick - s.now + (missing - 1) * interval) };
}
  ...
  if (s.tutorial >= 10 && s.now >= s.nextArrivalAt) {
    {
      ...
      const population = s.actors.filter(
        (a) => ['fighter', 'wizard', 'healer'].includes(a.role) && a.status !== 'dead',
      );
      if (
        ...
        s.actors.push(person);
      }
    }
    s.arrivalSequence =
      (s.arrivalSequence ?? Math.max(0, Math.floor(s.nextArrivalAt / HOUR) - 1)) + 1;
    s.nextArrivalAt = s.now + (rule(s, 'arrivalHours') * HOUR) / s.spawnPoints;
    admit(s);
  }
}
  ...
  if (s.researchJob && s.now >= s.researchJob.end) {
    ...
    if (id === 'staffSpeed')
      for (const a of s.actors.filter((a) =>
        ...
        if (a.task) {
          a.task.until = s.now + Math.ceil(Math.max(0, a.task.until - s.now) / 2);
          if (a.task.arriveAt !== undefined)
            a.task.arriveAt = s.now + Math.ceil(Math.max(0, a.task.arriveAt - s.now) / 2);
        }
        if (a.returnUntil)
...
export function advanceTo(state: GameState, target: number, active = false): GameState {
  ...
  // No rules run between fixed ticks. Retain entity references so memoized UI
  // and derived calculations stay cached until something actually changes.
  if (!state.opened || target === state.now) return state;
  if (target < state.nextTick && !active) return { ...state, now: target };
  const s: GameState =
    target < state.nextTick
...
function tutorial(s: GameState) {
  ...
  switch (s.tutorial) {
    ...
    case 8:
      ...
      s.actors.push(actor(s, 'fighter'));
      s.nextArrivalAt = s.now + rule(s, 'arrivalHours') * HOUR;
      log(s, 'One fighter has arrived at the town spawn. Invite the rest of their party.');
      break;
    case 9:
      for (const role of ['fighter', 'wizard', 'healer'] as Role[]) s.actors.push(actor(s, role));
      admit(s);
      unlock(s, 'training');
...
export function command(state: GameState, action: Command): GameState {
  ...
  switch (action.type) {
    ...
    case 'buySpawnPoint':
      ...
      s.spawnPoints++;
      s.nextArrivalAt = Math.min(
        s.nextArrivalAt,
        s.now + (rule(s, 'arrivalHours') * HOUR) / s.spawnPoints,
      );
