...
test('eight-day simulation preserves resources, population caps, and completes construction', () => {
  ...
  for (const [role, cap] of [
    // Fighters returning from recovery can exceed their spawn cap; nobody is removed.
    ['wizard', 20],
    ['healer', 10],
  ] as const)
...
test('ineligible legacy town members leave rather than silently waiting for training', () => {
  const s = demoState();
  s.parties = [];
  for (const f of s.floors) f.encounters = [];
  const a = s.actors.find((a) => a.role === 'fighter')!;
  ...
  a.trainingCount = 0;
  const next = advanceTo(s, s.nextTick);
  assert.ok(!next.actors.some((x) => x.id === a.id));
  a.trainingCount = 3;
  assert.equal(trainingEligible(s, a), false);
});
