...
test('outer caves have extra clearance while interior slots remain evenly spaced', async () => {
  const { EXIT_CENTER, REST_CENTER, ENTRY_CENTER, slotX, CELL_WIDTH, DOOR_GAP } =
    await import('../src/game/layout');
  const centers = [
    EXIT_CENTER,
    ...Array.from({ length: 12 }, (_, i) => slotX(i) + 15),
    REST_CENTER,
    ENTRY_CENTER,
  ];
  assert.ok(Math.abs(centers[1]! - centers[0]! - CELL_WIDTH - DOOR_GAP) < 0.000001);
  assert.ok(Math.abs(centers[14]! - centers[13]! - CELL_WIDTH - DOOR_GAP) < 0.000001);
  for (let i = 2; i <= 13; i++)
    assert.ok(Math.abs(centers[i]! - centers[i - 1]! - (CELL_WIDTH - DOOR_GAP / 6)) < 0.000001);
});
